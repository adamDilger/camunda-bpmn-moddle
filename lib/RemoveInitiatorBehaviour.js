...
/**
 * Remove `activiti:initiator` property when a startEvent is moved to or created within a subProcess.
 */
function RemoveInitiatorBehaviour(
  ...
  this.postExecuted(['shape.create','shape.move'], function(context) {
    ...
    if (is(shape, 'bpmn:StartEvent') && businessObject.get('activiti:initiator') !== undefined) {

      // if subProcess becomes the new parent
      if ((is(newParent, 'bpmn:SubProcess'))) {

        // remove initiator property
        modeling.updateProperties(shape, { 'activiti:initiator': undefined });
